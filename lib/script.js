...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add showReceipt function here
    function showReceipt(rideId) {
        // Find the ride by ID or use the provided ride object
        let ride;
        if (typeof rideId === 'object') {
            ride = rideId;
        } else {
            ride = rides.find(r => r.id === rideId);
        }
        ...
        if (receiptModal) {
            ...
            document.getElementById('receipt-pickup').textContent = ride.pickup;
            document.getElementById('receipt-dropoff').textContent = ride.dropoff;
            document.getElementById('receipt-fare').textContent = `$${parseFloat(ride.fare).toFixed(2)}`;
            
            // Show discount info if applicable
            const discountSection = document.getElementById('receipt-discount-section');
            if (ride.discount > 0 && discountSection) {
                discountSection.style.display = 'block';
                document.getElementById('receipt-original-fare').textContent = 
                    `$${parseFloat(ride.originalFare).toFixed(2)}`;
                document.getElementById('receipt-discount').textContent = 
                    `${ride.discount}% ($${parseFloat(ride.discountAmount).toFixed(2)})`;
