...
class TeslaAPI {
  ...
  getAuthorizationUrl() {
    ...
    // Use the correct scopes for Tesla Fleet API
    // According to Tesla API documentation: openid email offline_access vehicle_device_data vehicle_cmds
  ...
  async exchangeCodeForTokens(code) {
    ...
    // Log the full request parameters (excluding sensitive data)
    console.log('Token exchange request parameters:', params.toString().replace(/(client_secret=)[^&]+/, '$1[REDACTED]'));
    ...
    try {
      ...
      if (!response.ok) {
        console.error('Tesla API Error:', data);
  ...
  async refreshAccessToken() {
    ...
    // Update parameters to match Tesla's refresh token requirements
    const params = new URLSearchParams({
  ...
  async getVehicleData(vehicleId) {
    // Use id_s instead of id as per Tesla API requirements
    // The id_s field should be used for API calls to avoid 404 errors
  ...
  /**
   * Get token using client credentials OAuth flow
   * Note: This method is only for testing purposes as Tesla may not support client credentials flow
   * @returns {Promise<Object>} Token response
   */
  async getClientCredentialsToken() {
    ...
    console.log('Request parameters:', params.toString().replace(/(client_secret=)[^&]+/, '$1[REDACTED]'));
    ...
    try {
      ...
      if (!response.ok) {
        console.error('Tesla API Error:', data);
        throw new Error(data.error_description || 'Failed to get client credentials token');
      }
